...
local tn_style_format = {
  ...
  progress_text = '{\\1c&%s&\\3c&H%s&\\1a&H%X&\\3a&H%X&\\blur0.25\\fs18\\fscx%f\\fscy%f\\xbord%f\\ybord%f\\fn'
    .. user_opts.font_mono
    .. '}',
  text_timer = '%.2ds',
  text_progress = '%.3d/%.3d',
...
local function show_thumbnail(seek_percent)
  ...
  if
    path and not (thumbnail.visible and thumbnail.x_last == x and thumbnail.y_last == y and thumbnail.path_last == path)
  then
...
local function display_tn_osc(seek_y, seek_percent, ass)
  ...
  if tn_osc.display_progress.current then
    ...
    local processing = tn_osc_stats.processing > 0 and (tn_style_format.text_progress2):format(tn_osc_stats.processing)
      or ''
    ass_new(
      ass,
      tn_osc.position.x,
...
mp.register_script_message(message.osc.update, function(json)
  ...
  if new_data.thumbnails and tn_state then
    ...
    for time_string, status in pairs(new_data.thumbnails) do
      ...
      tn_thumbnails_indexed[index] = ready and join_paths(tn_state.cache_dir, time_string) .. tn_state.cache_extension
        or status
...
local osc_styles = {
  ...
  timecodes = ('{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs20\\b%d\\fn%s}'):format(
    user_opts.font_bold,
    user_opts.font_mono
  ),
  vidtitle = ('{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs12\\b%d\\q2\\fn%s}'):format(
    user_opts.font_bold,
    user_opts.font
  ),
  ...
  timecodesBar = ('{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs27\\b%d\\fn%s}'):format(
    user_opts.font_bold,
    user_opts.font_mono
  ),
  timePosBar = ('{\\blur0.54\\bord%s\\1c&HFFFFFF\\3c&H000000\\fs27\\b%d\\fn%s}'):format(
    user_opts.tooltipborder,
    user_opts.font_bold,
    user_opts.font_mono
  ),
  vidtitleBar = ('{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\b%d\\q2\\fn%s}'):format(
    user_opts.font_bold,
    user_opts.font
  ),

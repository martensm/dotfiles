...
local options = {
  ...
  -- Note that sponsored segments may occasionally be inaccurate if this is turned off
...
local parse_update_interval = function()
...
local create_chapter = function(chapter_title, chapter_time)
  local chapters = mp.get_property_native('chapter-list')
  local duration = mp.get_property_native('duration')
  table.insert(
    chapters,
    { title = chapter_title, time = (duration == nil or duration > chapter_time) and chapter_time or duration - 0.001 }
  )
  table.sort(chapters, time_sort)
  mp.set_property_native('chapter-list', chapters)
end

local process = function(uuid, t, new_ranges)
...
local getranges = function(_, exists, db, more)
...
local fade_audio = function(step)
...
local select_category = function(selected)
...
local submit_segment = function(category)
  if not youtube_id then
    return
  end
  local start_time = math.min(segment.a, segment.b)

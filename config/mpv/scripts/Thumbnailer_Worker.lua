...
mp.command_native({
  ...
  format_json({ name = script_name, script_path = debug.getinfo(1).source }),
})
...
local function add_timeout(args)
  ...
  if OPERATING_SYSTEM == OS_MAC then
    add_args(
      args,
      worker_options.exec_path .. 'gtimeout',
      ('--kill-after=%d'):format(timeout + 1),
      ('%d'):format(timeout + 3)
    )
  elseif OPERATING_SYSTEM == OS_NIX then
    add_args(
      args,
      worker_options.exec_path .. 'timeout',
      ('--kill-after=%d'):format(timeout + 1),
      ('%d'):format(timeout + 3)
    )
  end
...
local function create_mpv_command(time, output, force_accurate_seek)
  ...
  if args then
    ...
    args[worker_extra.index_fastseek] = '--demuxer-lavf-o-set=fflags='
      .. (accurate_seek and '+discardcorrupt+nobuffer' or '+fastseek+discardcorrupt+nobuffer')
    ...
    worker_extra.index_fastseek = concat_args(
      args,
      '--demuxer-lavf-o-set=fflags=',
      accurate_seek and '+discardcorrupt+nobuffer' or '+fastseek+discardcorrupt+nobuffer'
    )
    worker_extra.index_accurate = concat_args(args, '--hr-seek=', accurate_seek and 'yes' or 'no')
    worker_extra.index_skip_loop = concat_args(
      args,
      '--vd-lavc-skiploopfilter=',
      accurate_seek and 'nonref' or 'nonkey'
    )
...
local function create_ffmpeg_command(time, output, force_accurate_seek)
  ...
  local accurate_seek = force_accurate_seek or worker_options.accurate_seek or is_last_thumbnail or state.delta < 3
  if args then
    args[worker_extra.index_fastseek] = accurate_seek and '+discardcorrupt+nobuffer'
      or '+fastseek+discardcorrupt+nobuffer'
    ...
    worker_extra.index_fastseek = add_args(
      args,
      '-fflags',
      accurate_seek and '+discardcorrupt+nobuffer' or '+fastseek+discardcorrupt+nobuffer'
    )
...
local function create_thumbnail(time, fullpath)
  return (run_subprocess(worker_extra.create_command(time, fullpath, false)) and check_existing(fullpath))
    or (run_subprocess(worker_extra.create_command(time, fullpath, true)) and check_existing(fullpath))
end

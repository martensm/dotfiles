...
local io, math, os, table = io, math, os, table
...
local function subprocess_result(sub_success, result, mpv_error, subprocess_name, start_time)
  ...
  if result then
    cmd_status, cmd_stdout, cmd_stderr, cmd_error, cmd_killed =
      result.status, result.stdout, result.stderr, result.error_string, result.killed_by_us
  end
  ...
  if success then
    msg.debug(
      ...
      cmd_status_string,
      '| Time:',
      ('%ds'):format(os.difftime(os.time(), start_time))
    )
  else
    msg.error(
      'Subprocess',
...
local function run_subprocess(command, name)
  ...
  local result, mpv_error = mp.command_native({
    name = 'subprocess',
    args = command,
    playback_only = false,
    capture_stdout = true,
...
local function exec_exist(name, exec_path)
  ...
  local env_path = exec_path ~= '' and exec_path
    or ((os.getenv('PWD') or mp.get_property('working-directory')) .. delim .. os.getenv('PATH'))
...
local function create_dir(path)
  return dir_exist(path)
    or run_subprocess(OPERATING_SYSTEM == OS_WIN and { 'cmd', '/e:on', '/c', 'mkdir', path } or { 'mkdir', '-p', path })
end
...
local function delete_dir(path)
  ...
  return run_subprocess(
    OPERATING_SYSTEM == OS_WIN and { 'cmd', '/e:on', '/c', 'rd', '/s', '/q', path } or { 'rm', '-r', path }
  )
end
...
local user_opts = {
  ...
  auto_delete = 0, -- Delete the thumbnail cache. Use at your own risk. 0=No, 1=On file close, 2=When quitting
...
local function worker_set_options()
  return {
    encoder = (not state.is_remote and user_opts.use_ffmpeg and exec_exist('ffmpeg', user_opts.exec_path)) and 'ffmpeg'
      or 'mpv',
...
local function workers_queue()
  ...
  for i, worker in ipairs(workers_indexed) do
    ...
    mp.command_native_async(
      { 'script-message-to', worker, message.worker.queue, format_json(worker_data) },
      function() end
    )
...
local function osc_delta_update(flush)
  ...
  if
    (time_since_last_update >= (4.00 * user_opts.update_time))
    or (time_since_last_update >= (2.00 * user_opts.update_time) and thumbnails_new_count >= state.worker_buffer)
    or (time_since_last_update >= (1.00 * user_opts.update_time) and thumbnails_new_count >= state.osc_buffer)
    or thumbnails_new_count >= floor(state.tn_per_worker - 1)
    or flush
  then
...
local function osc_update_count(time_string, status)
  local existing = thumbnails[time_string]
  if existing then
...
local function calculate_scale()
  ...
  if osc_opts then
    local scale = (saved_state.fullscreen ~= nil and saved_state.fullscreen) and osc_opts.scalefullscreen
      or osc_opts.scalewindowed
...
local function calculate_geometry(scale)
  ...
  geometry.dimension, geometry.width, geometry.height, geometry.dw, geometry.dh =
    dimension, width, height, video_params.dw, video_params.dh
  if not video_params.rotate then
    return geometry
  end
  geometry.rotate = (video_params.rotate - saved_state.initial_rotate) % 360
  geometry.is_rotated = not (
      (((video_params.rotate - saved_state.initial_rotate) % 180) ~= 0) == saved_state.meta_rotated
    ) --xor
  return geometry
end
...
local function reset_all(keep_saved, keep_osc_data)
  ...
  msg.debug(
    'Reset (' .. (keep_saved and 'Soft' or 'Hard') .. ', ' .. (keep_osc_data and 'OSC-Partial' or 'OSC-All') .. ')'
  )
end
...
mp.observe_property('video-params', 'native', function(_, video_params)
  ...
  if
    initialized
    and saved_state
    and saved_state.video_params
    and saved_state.video_params.rotate
    and video_params.rotate
    and tostring(saved_state.video_params.rotate) ~= tostring(video_params.rotate)
  then
    reset_all(true)
    saved_state.video_params = video_params
    start()
...
mp.register_script_message(message.worker.registration, function(new_reg)
  ...
  if worker_reg.name and not workers[worker_reg.name] then
    ...
    if (is_empty(worker_script_path)) and not is_empty(worker_reg.script_path) then
      ...
      create_workers()
      msg.debug('Worker Script Path Received:', worker_script_path)
    end

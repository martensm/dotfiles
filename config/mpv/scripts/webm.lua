...
format_filename = function(startTime, endTime, videoFormat)
  ...
  local replaceTable = {
    ...
    ['%%M'] = (mp.get_property_native('aid') and not mp.get_property_native('mute') and hasAudioCodec) and '-audio'
      or '',
    ['%%R'] = (options.scale_height ~= -1) and '-' .. tostring(options.scale_height) .. 'p' or '-' .. tostring(
      mp.get_property_native('height')
    ) .. 'p',
    ['%%t%%'] = '%%',
  }
...
get_video_dimensions = function()
  ...
  if keep_aspect then
    ...
    _video_dimensions.top_left.x, _video_dimensions.bottom_right.x = split_scaling(
      window_w,
      scaled_width,
      zoom,
      align_x,
    ...
    _video_dimensions.top_left.y, _video_dimensions.bottom_right.y = split_scaling(
      window_h,
      scaled_height,
      zoom,
      align_y,
...
get_video_filters = function(format, region)
  ...
  if region and region:is_valid() then
    append(filters, {
      'lavfi-crop=' .. tostring(region.w) .. ':' .. tostring(region.h) .. ':' .. tostring(region.x) .. ':' .. tostring(
        region.y
      ),
    })
  end
  ...
  local _base_0 = {
    ...
    setValue = function(self, value)
      ...
      elseif 'list' == _exp_0 then
        ...
        if not set then
          return msg.warn(
            'Tried to set invalid value ' .. tostring(value) .. ' to ' .. tostring(self.displayText) .. ' option.'
          )
        end
      end
    end,
...
do
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:append(
        tostring(bold('2:')) .. ' set end time (current is ' .. tostring(seconds_to_time_string(self.endTime)) .. ')\\N'
      )
